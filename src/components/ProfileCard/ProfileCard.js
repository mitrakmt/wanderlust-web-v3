...
export default function ProfileCard({ user, userId, currentUserId, followUser, removeFollow, follows }) {
    ...
    return (
        <div className="relative flex flex-col items-center justify-center w-full px-0 md:px-4 py-6 bg-white border border-gray-200 rounded-lg shadow-md h-80 lg:w-70 xl:w-74 dark:bg-gray-800 dark:border-gray-700">
            {
                user.profile_image ?
                    <Image priority className="relative w-24 h-24 md:w-26 md:h-26 xl:w-30 xl:h-30 mt-8 mb-3 rounded-full shadow-lg" quality={80} width={50} height={30} src={user.profile_image} alt={user.name} />
